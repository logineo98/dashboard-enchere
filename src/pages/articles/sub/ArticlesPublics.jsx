...
const ArticlesPublics = () => {
    ...
    const column = [
        ...
        { name: "Propriétaire", selector: (row) => users?.map(user => user?._id === row?.sellerID && (user?.phone || user?.facebook?.first_name)), sortable: true, },
        ...
        { name: "Enchère", selector: (row) => row?.enchere_type === "public" ? "Public" : "...", sortable: true, },
    ...
    const tabsItems = [
        ...
        { label: "publier", size: encheres?.filter(enchere => !enchere?.trash && enchere?.enchere_type === "public" && enchere?.enchere_status === "published" && !ExpirationVerify(enchere?.expiration_time)).length || 0 },
        { label: "attente", size: encheres?.filter(enchere => !enchere?.trash && enchere?.enchere_type === "public" && enchere?.enchere_status === "pending" && !ExpirationVerify(enchere?.expiration_time)).length || 0 },
        { label: "rejetés", size: encheres?.filter(enchere => !enchere?.trash && enchere?.enchere_type === "public" && enchere?.enchere_status === "rejected" && !ExpirationVerify(enchere?.expiration_time)).length || 0 },
        { label: "terminés", size: encheres?.filter(enchere => !enchere?.trash && enchere?.enchere_type === "public" && (ExpirationVerify(enchere?.expiration_time) || enchere?.enchere_status === "closed")).length || 0 },
        { label: "corbeille", size: encheres?.filter(enchere => enchere?.trash && enchere?.enchere_type === "public").length || 0 }]
    ...
    useEffect(() => {
        switch (activeTab) {
            // case "tous": setData(encheres?.filter(enchere => !enchere?.trash && enchere?.enchere_type === "public")); break;
            case "publier": setData(encheres?.filter(enchere => !enchere?.trash && enchere?.enchere_status === "published" && enchere?.enchere_type === "public" && !ExpirationVerify(enchere?.expiration_time))); setClear(true); break;
            case "attente": setData(encheres?.filter(enchere => !enchere?.trash && enchere?.enchere_status === "pending" && enchere?.enchere_type === "public" && !ExpirationVerify(enchere?.expiration_time))); setClear(true); break;
            case "rejetés": setData(encheres?.filter(enchere => !enchere?.trash && enchere?.enchere_status === "rejected" && enchere?.enchere_type === "public" && !ExpirationVerify(enchere?.expiration_time))); setClear(true); break;
            case "terminés": setData(encheres?.filter(enchere => !enchere?.trash && enchere?.enchere_type === "public" && (ExpirationVerify(enchere?.expiration_time) || enchere?.enchere_status === "closed"))); setClear(true); break;
            case "corbeille": setData(encheres?.filter(enchere => enchere?.trash && enchere?.enchere_type === "public")); setClear(true); break;
            default: setData(encheres?.filter(enchere => !enchere?.trash && enchere?.enchere_status === "published" && enchere?.enchere_type === "public" && !ExpirationVerify(enchere?.expiration_time))); setClear(true); break;
        }
        setClear(c => !c)
    }, [encheres, activeTab]);
            ...
            switch (dropDown) {
                ...
                case "publier":
                    data?.forEach((enchere) => {
                        rows?.forEach((id) => {
                            if (enchere?._id === id) {
                                users?.forEach(user => { console.log(user?._id, " === ", enchere?.sellerID); if (user?._id === enchere?.sellerID) dispatch(send_notification({ title: "Article publié", body: `La mise en ligne de votre article "${enchere?.title}" a été approuvée. Il est maintenant disponible pour être encherir.`, to: user?.notification_token, data: null })) })
    ...
    const handleFilter = (e) => {
        const filteredData = data?.filter(enchere => {
            ...
            const started_priceMatches = enchere?.started_price?.toString().trim().toLowerCase().includes(searchString)
            ...
            const phoneMatches = users?.some(user => (user?._id === enchere?.sellerID) && user?.phone?.toString().trim().toLowerCase().includes(searchString))

            return phoneMatches || started_priceMatches || titleMatches || descriptionMatches || reserve_priceMatches || locationMatches || categoriesMatches
        })
